...
label(ax, 6.05, 2.39, r'$\cdots$', fs=8.0, color=gray_bdr)
label(ax, 6.05, 2.20, r'$Score(v_{m-1})$', fs=7.1, color=text_dark)
draw_arrow(ax, 7.08, 2.65, 6.68, 2.65, color='#C67008', lw=1.0)

# Step 3: Virtual Stake and Leader Election
label(ax, 1.15, 3.62, '(Step 3)', fs=8.2, color=text_dark, fw='bold')
label(ax, 1.15, 3.42, 'Virtual Stake And Election', fs=8.4, color=text_dark, fw='bold')
draw_box(ax, 3.55, 4.08, 1.55, 0.46, 'Economic Stake', fc=orange_lt, ec=orange, fs=8.1)
draw_box(ax, 3.55, 2.65, 1.80, 0.72, 'Virtual Stake\n$S_v$', fc=green_lt, ec=green, fs=8.8, lw=1.6)
draw_box(ax, 1.15, 2.65, 1.70, 0.68, '', fc=white, ec=navy, fs=8.8)
label(ax, 1.15, 2.79, 'Leader Election', fs=8.8, color=text_dark, fw='bold')
draw_text_tag(ax, 1.15, 2.47, 'RANDAO', 0.92, 0.22, fc=white, ec=navy)

draw_arrow(ax, 3.55, 3.76, 3.55, 3.12, color=orange, lw=1.2)
draw_arrow(ax, 5.48, 2.65, 4.62, 2.65, color=green, lw=1.2)
draw_arrow(ax, 2.58, 2.65, 2.00, 2.65, color=green, lw=1.2)

label(ax, 4.10, 3.50, r'$1-\omega$', fs=7.2, color=orange)
label(ax, 5.00, 2.88, r'$\omega$', fs=7.2, color=green)

project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
figures_dir = os.path.join(project_root, 'figures')
os.makedirs(figures_dir, exist_ok=True)

plt.savefig(os.path.join(figures_dir, 'overview_step3.png'), dpi=600,
