...
draw_node(ax, 5.5, ny, nr, sky, blue_mid, '$v_i$')
...
label(ax, 10.05, 1.22, 'bls', fs=6.8, color=navy, fw='bold', fst='italic')
...
label(ax, 5.35, -1.04, 'Chained-signature Verifiable Path Tracing', fs=10.2,
...
label(ax, 6.05, 3.04, r'$Score(v_0)$', fs=7.1, color=text_dark)
label(ax, 6.05, 2.81, r'$Score(v_i)$', fs=7.1, color=text_dark)
...
draw_box(ax, 3.55, 3.05, 1.80, 0.72, 'Virtual Stake\n$S_v$', fc=green_deep_lt, ec=green_deep, fs=8.8, lw=1.6)
...
draw_text_tag(ax, 1.15, 1.71, r'$R_{base} + \eta \cdot fee$', 1.34, 0.20, fc=white, ec=orange)
...
draw_text_tag(ax, 3.55, 1.69, r'$(1-\eta)\, fee$', 1.12, 0.20, fc=white, ec=orange)
