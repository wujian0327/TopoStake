...
const TOPOS_SIDECAR_MARKER: &[u8] = b"TOPOSTAKE";

fn transaction_contains_topostake_marker<E: EthSpec>(block: &SignedBeaconBlock<E>) -> bool {
...
impl<T: BeaconChainTypes> NetworkBeaconProcessor<T> {
    ...
    async fn process_gossip_unverified_block(
        ...
        let topostake_sidecar_marker = transaction_contains_topostake_marker(block.as_ref());
        let topostake_sidecar_marker_label = if topostake_sidecar_marker {
            "present"
        } else {
            "absent"
        };
        ...
        let block_root = if let Ok(verified_block) = &verification_result {
            ...
            metrics::inc_counter_vec(
                &metrics::TOPOSTAKE_GOSSIP_BLOCK_ARRIVALS_TOTAL,
                &[peer_client_kind, topostake_sidecar_marker_label],
            );
            metrics::set_gauge_vec(
                &metrics::TOPOSTAKE_GOSSIP_BLOCK_ARRIVAL_DELAY_MILLISECONDS,
                &[peer_client_kind, topostake_sidecar_marker_label],
                block_delay.as_millis() as i64,
            );
            metrics::set_gauge_vec(
                &metrics::TOPOSTAKE_GOSSIP_BLOCK_INFO,
                &[
                    &block_root_label,
                    &slot_label,
                    &proposer_index_label,
                    peer_client_kind,
                    topostake_sidecar_marker_label,
                ],
                1,
            );
            verified_block.block_root
        ...
        let verified_block = match verification_result {
            Ok(verified_block) => {
                    ...
                    .and_then(|now| now.checked_sub(seen_duration))
                {
                    ...
                    metrics::set_gauge_vec(
                        &metrics::TOPOSTAKE_GOSSIP_BLOCK_VERIFICATION_DELAY_MILLISECONDS,
                        &[peer_client_kind, topostake_sidecar_marker_label],
                        duration.as_millis() as i64,
                    );
                }
