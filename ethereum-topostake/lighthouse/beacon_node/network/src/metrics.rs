...
/*
 * TopoStake propagation instrumentation.
 *
 * These metrics are enabled by default in the TopoStake Lighthouse image. They
 ...
 */
pub static TOPOSTAKE_GOSSIP_BLOCK_ARRIVALS_TOTAL: LazyLock<Result<IntCounterVec>> = LazyLock::new(
    || {
        try_create_int_counter_vec(
            "topostake_gossip_block_arrivals_total",
            "TopoStake instrumentation: count of gossip blocks observed by peer client and sidecar marker presence.",
            &["peer_client", "sidecar_marker"],
        )
    },
);
pub static TOPOSTAKE_GOSSIP_BLOCK_ARRIVAL_DELAY_MILLISECONDS: LazyLock<Result<IntGaugeVec>> =
    LazyLock::new(|| {
        try_create_int_gauge_vec(
            "topostake_gossip_block_arrival_delay_milliseconds",
            "TopoStake instrumentation: latest gossip block arrival delay from slot start in milliseconds.",
            &["peer_client", "sidecar_marker"],
        )
    });
pub static TOPOSTAKE_GOSSIP_BLOCK_VERIFICATION_DELAY_MILLISECONDS: LazyLock<Result<IntGaugeVec>> =
    LazyLock::new(|| {
        try_create_int_gauge_vec(
            "topostake_gossip_block_verification_delay_milliseconds",
            "TopoStake instrumentation: latest delay from gossip receipt to propagation verification in milliseconds.",
            &["peer_client", "sidecar_marker"],
        )
    });
pub static TOPOSTAKE_GOSSIP_BLOCK_INFO: LazyLock<Result<IntGaugeVec>> = LazyLock::new(|| {
    try_create_int_gauge_vec(
        "topostake_gossip_block_info",
        "TopoStake instrumentation: last observed gossip block metadata. Labels are high-cardinality and intended for devnet-only use.",
        &[
            "block_root",
            "slot",
            "proposer_index",
