...
use typenum::U48;
use types::{
    ChainSpec, EthSpec, GRAFFITI_BYTES_LEN, Graffiti, Hash256, TopoStakeInlineEvidenceRecord,
};
...
pub fn topostake_tx_evidence_records_from_json<E: EthSpec>(
    evidence: Option<serde_json::Value>,
    spec: &ChainSpec,
) -> VariableList<TopoStakeInlineEvidenceRecord<E>, E::MaxBlobCommitmentsPerBlock> {
    let Some(evidence) = evidence else {
        return VariableList::empty();
    };
    serde_json::from_value::<TopoStakeBlockEvidence>(evidence)
        .ok()
...
pub fn topostake_tx_evidence_root_from_json(
    evidence: Option<serde_json::Value>,
) -> Option<Hash256> {
...
impl TopoStakeBlockEvidence {
    fn inline_records<E: EthSpec>(
        self,
        spec: &ChainSpec,
    ) -> Result<Vec<TopoStakeInlineEvidenceRecord<E>>, String> {
        ...
        let max_len = spec.topostake_config.max_path_evidence_len as usize;
...
mod tests {
    ...
    use crate::graffiti_calculator::{GraffitiSettings, topostake_evidence_root_to_graffiti};
    ...
    #[test]
    fn topostake_evidence_root_graffiti_uses_short_root_tag() {
        let graffiti = topostake_evidence_root_to_graffiti(
