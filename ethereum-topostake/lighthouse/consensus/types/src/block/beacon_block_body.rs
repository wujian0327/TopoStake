...
pub struct TopoStakeInlineEvidenceRecord<E: EthSpec> {
    pub tx_hash: Hash256,
    pub epoch: u64,
...
pub struct BeaconBlockBody<E: EthSpec, Payload: AbstractExecPayload<E> = FullPayload<E>> {
    ...
    /// Devnet-only TopoStake commitment. This field intentionally participates in SSZ and
    /// tree-hash roots in the custom fork.
    pub topostake_evidence_root: Hash256,
    /// Prompt 25 bounded block-inline TopoStake path records. These records are committed by the
    /// block body root and let block processing avoid external evidence sidecar availability for
    /// the common path.
    pub topostake_evidence_records:
...
impl<E: EthSpec> BeaconBlockBodyBellatrix<E, FullPayload<E>> {
    pub fn clone_as_blinded(&self) -> BeaconBlockBodyBellatrix<E, BlindedPayload<E>> {
        ...
        BeaconBlockBodyBellatrix {
            ...
            graffiti: *graffiti,
            topostake_evidence_root: *topostake_evidence_root,
            topostake_evidence_records: topostake_evidence_records.clone(),
