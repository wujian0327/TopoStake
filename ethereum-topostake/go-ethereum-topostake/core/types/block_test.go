...
func TestTopoStakeSettlementRecordsSurviveBlockCopies(t *testing.T) {
	header := &Header{
	...
	records := []TopoStakeSettlementRecord{{
		FinalizedEpoch: 1,
		Epoch:          0,
		Role:           "relay",
		ValidatorIndex: 7,
	...
	block := NewBlockWithHeader(header).WithBody(Body{
		TopoStakeSettlementRecords: records,
	})

	if got := len(block.TopoStakeSettlementRecords()); got != 1 {
		t.Fatalf("records lost in WithBody: got %d, want 1", got)
	}
	if got := len(block.WithWitness(&ExecutionWitness{}).TopoStakeSettlementRecords()); got != 1 {
		t.Fatalf("records lost in WithWitness: got %d, want 1", got)
	}
	if got := len(block.WithSeal(header).TopoStakeSettlementRecords()); got != 1 {
