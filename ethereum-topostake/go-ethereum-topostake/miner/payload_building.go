...
import (
	...
	"github.com/ethereum/go-ethereum/eth/topostake"
...
func (payload *Payload) Resolve() *engine.ExecutionPayloadEnvelope {
	...
	if payload.full != nil {
		...
		attachTopoStakeBlockEvidence(envelope, payload.full, payload.fullReceipts)
	...
	attachTopoStakeBlockEvidence(envelope, payload.empty, nil)
...
func attachTopoStakeBlockEvidence(envelope *engine.ExecutionPayloadEnvelope, block *types.Block, receipts []*types.Receipt) {
	if envelope == nil || block == nil {
		return
	}
	evidence := topostake.DefaultStore().BuildBlockEvidence(
		block.Hash(),
		block.NumberU64(),
		block.Transactions(),
		receipts,
	...
	if err != nil {
		log.Warn("Failed to encode TopoStake payload evidence", "block", block.Hash(), "err", err)
		return
	}
	envelope.TopoStakeBlockEvidence = encoded
}
