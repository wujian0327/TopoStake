...
func (miner *Miner) generateWork(genParam *generateParams, witness bool) *newPayloadResult {
	...
	topostake.DefaultStore().RecordBlockEvidenceWithReceipts(block.Hash(), block.NumberU64(), block.Transactions(), work.receipts, block.BaseFee(), block.Coinbase())
...
func (miner *Miner) prepareWork(genParams *generateParams, witness bool) (*environment, error) {
	...
	if root := topostake.DefaultStore().PendingSettlementRoot(); root != (common.Hash{}) {
		header.Extra = topostake.SettlementExtraFromRoot(root)
	}
