...
func (p *Peer) sendPooledTransactionHashes(hashes []common.Hash, types []byte, sizes []uint32) error {
	...
	return p2p.Send(p.rw, NewPooledTransactionHashesMsg, NewPooledTransactionHashesPacket{
		...
		Sizes:     sizes,
		Hashes:    hashes,
		TopoStake: metadataBatch(hashes, p.ID(), p.RemoteAddr().String()),
	})
}
...
func metadataBatch(hashes []common.Hash, peerID string, remoteAddr string) []TopoStakeTxMetadata {
	store := topostake.DefaultStore()
	if !store.Enabled() {
		return nil
	}
	...
	batch := make([]TopoStakeTxMetadata, len(raw))
	for i := range raw {
		batch[i] = TopoStakeTxMetadata(raw[i])
	}
	return batch
}
