...
func handleNewPooledTransactionHashes(backend Backend, msg Decoder, peer *Peer) error {
	...
	if len(ann.TopoStake) > 0 && len(ann.TopoStake) != len(ann.Hashes) {
		topostake.DefaultStore().RecordInbound(nil, nil)
		return fmt.Errorf("NewPooledTransactionHashes: invalid topostake metadata len %v for hashes %v", len(ann.TopoStake), len(ann.Hashes))
	}
	recordInboundMetadata(ann.Hashes, ann.TopoStake)
...
func handlePooledTransactions(backend Backend, msg Decoder, peer *Peer) error {
	...
	if len(txs.TopoStake) > 0 && len(txs.TopoStake) != len(txs.PooledTransactionsResponse) {
		topostake.DefaultStore().RecordInbound(nil, nil)
...
func recordInboundMetadata(hashes []common.Hash, batch []TopoStakeTxMetadata) {
	if len(batch) == 0 {
		return
	}
