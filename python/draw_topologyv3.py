...
def draw_reward_stake_icons(ax, zoom=0.070):
    reward_positions = [
        (0.68, 1.68),
        (1.08, -1.62),
        (-1.94, 0.26),
    ...
    for xy in reward_positions:
        image = OffsetImage(icon_images['relay_reward'], zoom=zoom)
...
def draw_node_reward_icons(ax, pos, node_offsets, zoom=0.060):
    for node, (dx, dy) in node_offsets.items():
        image = OffsetImage(icon_images['relay_reward'], zoom=zoom)
        artist = AnnotationBbox(
            image,
            (pos[node][0] + dx, pos[node][1] + dy),
            frameon=False,
            box_alignment=(0.5, 0.5),
            zorder=7,
        )
...
def draw_summary_card(fig, x, y, width, height, title, bullets,
                      ...
                      bullet_indent=0.22, text_indent=0.50):
    ...
    first_y = y + height * 0.565
    line_gap = height * 0.250
...
fig.subplots_adjust(left=0.035, right=0.965, top=0.94, bottom=0.245, wspace=0.08)
